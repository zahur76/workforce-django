        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if sick_start_year != sick_end_year:
                    messages.error(
                        request, f"Entry allowed for 1 year only!, e.g 20{actual_year}"
                    )
                    return redirect(reverse("staff_details", args=[staff_id]))
                    ...
                    if sick_leave_periods or annual_leave_periods:
                        ...
                        for leave in sick_leave_periods:
                            ...
                            for x in range(0, sick_days):
                                day_list.append(
                                    leave.start_date + datetime.timedelta(days=x)
                                )
                        ...
                        for x in range(0, (sick.end_date - sick.start_date).days + 1):
                            if (
                                sick.start_date + datetime.timedelta(days=x)
                            ) in day_list:
                                count += 1
                            else:
                                count += 0
                    ...
                    if sick_leave_periods or annual_leave_periods:
                        ...
                        if count != 0:
                            ...
                            difference = (sick.end_date - sick.start_date).days + 1
                            staff.sick_leave_remaining = (
                                staff.sick_leave_remaining - difference
                            )
                            staff.save()
                            sick.staff = staff
                            sick.days = difference
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if annual_start_year != annual_end_year:
                    messages.error(
                        request, f"Entry allowed for 1 year, e.g 20{actual_year}!"
                    )
                    return redirect(reverse("staff_details", args=[staff_id]))
                    ...
                    actual_year != annual_start_year and actual_year != annual_end_year
                ):
                    if sick_leave_periods or annual_leave_periods:
                        count = 0
                        day_list = []
                        ...
                        for x in range(
                            0, (annual.end_date - annual.start_date).days + 1
                        ):
                            if (
                                annual.start_date + datetime.timedelta(days=x)
                            ) in day_list:
                                count += 1
                            else:
                                count += 0
                    ...
                    if sick_leave_periods or annual_leave_periods:
                        ...
                        if count != 0:
                            ...
                            staff.annual_leave_remaining = (
                                staff.annual_leave_remaining - difference
                            )
                            staff.save()
                            annual.staff = staff
                            annual.days = difference
...
def sick_leave_taken(request, staff_id):
    ...
    if not request.user.is_superuser:
        ...
        if query == "All":
            ...
            sick_leave = sick_leave.filter(
                staff__id=staff_id, start_date__year=query
            ).order_by("start_date")
        actual_year = datetime.datetime.now().strftime("%y")
        context = {
            "staff": staff,
...
def annual_leave_taken(request, staff_id):
    ...
    if not request.user.is_superuser:
        ...
        if query == "All":
            annual_leave = annual_leave.filter(staff__id=staff_id).order_by(
                "start_date"
            )
        else:
            annual_leave = annual_leave.filter(
                staff__id=staff_id, start_date__year=query
            ).order_by("start_date")
        actual_year = datetime.datetime.now().strftime("%y")
        context = {
...
def sick_modify(request, sick_id):
    ...
    if not request.user.is_superuser:
        ...
        sick_leave_periods = (
            SickLeave.objects.all().filter(staff__id=staff.id).exclude(id=sick_id)
        )
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if sick_applied_year_start != sick_applied_year_end:
                    ...
                    actual_year != sick_applied_year_start
                    and actual_year != sick_applied_year_end
                ):
                    if sick_leave_periods or annual_leave_periods:
                        count = 0
                        day_list = []
                        ...
                        for x in range(
                            0,
                            (modified_sick.end_date - modified_sick.start_date).days
                            + 1,
                        ):
                            if (
                                sick.start_date + datetime.timedelta(days=x)
                        ...
                        if count != 0:
                            messages.error(request, "Error Duplicate dates!")
                            return redirect(
                                reverse("sick_leave_taken", args=[sick.staff.id])
                            )
                        else:
                            modified_difference = (
                                modified_sick.end_date - modified_sick.start_date
                            ).days + 1
                            staff.save()
                ...
                messages.error(
                    request,
                    "Sick leave could not be modified. \
                        Please ensure the form is invalid.",
                )
                return redirect(reverse("sick_leave_taken", args={sick.staff.id}))
        else:
...
def annual_modify(request, annual_id):
    ...
    if not request.user.is_superuser:
        ...
        annual_leave_periods = (
            AnnualLeave.objects.all().filter(staff__id=staff.id).exclude(id=annual_id)
        )
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if annual_start_year != annual_end_year:
                    ...
                    return redirect(
                        reverse("annual_leave_taken", args=[annual.staff.id])
                    )
                    ...
                    if sick_leave_periods or annual_leave_periods:
                        ...
                        for x in range(
                            0,
                            (modified_annual.end_date - modified_annual.start_date).days
                            + 1,
                        ):
                            if (
                                annual.start_date + datetime.timedelta(days=x)
                        ...
                        if count != 0:
                            ...
                            modified_difference = (
                                modified_annual.end_date - modified_annual.start_date
                            ).days + 1
                            staff.save()
                ...
                messages.error(
                    request,
                    "Annual leave could not be modified. \
                        Please ensure the form is invalid.",
                )
...
def sick_data(request):
    ...
    if not request.user.is_superuser:
        ...
        if query != "All":
            all_sick = (
                SickLeave.objects.all()
                .filter(start_date__year=query)
                .order_by("start_date")
            )
            sick_data = {
                "Jan": 0,
...
def leave_planner(request):
    ...
    if not request.user.is_superuser:
        ...
        month_leave = (
            AnnualLeave.objects.all()
            .filter(start_date__month=month, start_date__year=query)
            .order_by("start_date")
        )
        month_dates = {}
        for x in range(1, 32):
...
def sick_reset(request):
    if not request.user.is_superuser:
        ...
        for sick in staff_sick:
            total_sick = 0
            total_sick_current_year = SickLeave.objects.all().filter(
                staff__id=sick.id, start_date__year=actual_year
            )
...
def leave_reset(request):
    if not request.user.is_superuser:
        ...
        for leave in staff_leave:
            total_leave = 0
            total_leave_current_year = AnnualLeave.objects.all().filter(
                staff__id=leave.id, start_date__year=actual_year
            )
